...
$target_pathh = $i.'__'. date('d_m_Y').'time'.date('H_i_s'). "." . pathinfo($target_file,PATHINFO_EXTENSION);     // Set the target path with a new name of image.
$j = $j + 1;      // Increment the number of uploaded images according to the files in array.
if (($_FILES["ppicture"]["size"][$i] < 2000000)     // Approx. 1000kb files can be uploaded.
&& in_array($file_extension, $validextensions)) {
if (move_uploaded_file($_FILES['ppicture']['tmp_name'][$i], "product_images/".$target_pathh)) {
